...
# Usage: iwr -useb https://aimlapi.com/openclaw/install.ps1 | iex
#        & ([scriptblock]::Create((iwr -useb https://aimlapi.com/openclaw/install.ps1))) -Tag beta -NoOnboard -DryRun
...
Write-Host ""
Write-Host "  OpenClaw + AI/ML API Installer" -ForegroundColor Cyan
Write-Host ""
...
function Install-OpenClaw {
    ...
    # Use openclaw-aimlapi package for AI/ML API builds
    $packageName = "openclaw-aimlapi"
    ...
    try {
        ...
        if ($LASTEXITCODE -ne 0) {
            ...
            if ($npmOutput -match "spawn git" -or $npmOutput -match "ENOENT.*git") {
                ...
                Write-Host "  iwr -useb https://aimlapi.com/openclaw/install.ps1 | iex" -ForegroundColor Cyan
            }
...
function Install-OpenClawFromGit {
    ...
    $repoUrl = "https://github.com/aimlapi/openclaw-aimlapi.git"
    $repoBranch = "feature/add-aimlapi-models-provider"
    ...
    if (-not (git -C $RepoDir status --porcelain 2>$null)) {
        ...
        if (git -C $RepoDir show-ref --verify --quiet "refs/heads/$repoBranch") {
            git -C $RepoDir checkout $repoBranch 2>$null
        } else {
            if (git -C $RepoDir show-ref --verify --quiet "refs/remotes/origin/$repoBranch") {
                git -C $RepoDir checkout -b $repoBranch "origin/$repoBranch" 2>$null
            } else {
                Write-Host "[!] Branch not found on origin: $repoBranch" -ForegroundColor Yellow
                try {
                    $currentBranch = (git -C $RepoDir rev-parse --abbrev-ref HEAD 2>$null).Trim()
                    if ($currentBranch) {
                        Write-Host "[!] Continuing on current branch: $currentBranch" -ForegroundColor Yellow
                    }
                } catch {}
            }
        }
    } else {
        Write-Host "[!] Repo is dirty; skipping fetch/checkout of $repoBranch" -ForegroundColor Yellow
    }
...
function Main {
    ...
    Write-Host ""
    Write-Host "AI/ML API provides 300+ AI models including Deepseek, Gemini, ChatGPT. The models run at enterprise-grade rate limits and uptimes" -ForegroundColor Gray
    Write-Host ""
    Write-Host "Get API key: https://aimlapi.com/app/keys/?utm_source=openclaw-aimlapi&utm_medium=github&utm_campaign=integration" -ForegroundColor Gray
    Write-Host "Provider docs: https://docs.openclaw.ai/start/faq" -ForegroundColor Gray
    Write-Host ""
